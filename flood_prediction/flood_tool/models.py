...
class Models:
                ...
                (
                    ...
                    FunctionTransformer(
                        lambda X: self._drop_columns(
                            X, ["localAuthority", "nearestWatercourse", "soilType"]
                        ),
                        validate=False,
                    ),
                ),
    ...
    def _add_interaction_terms(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        bins_elevation = pd.qcut(
            data["elevation"], q=4, labels=["Low", "Mid", "High", "Very High"]
        )
        # bins_distance = pd.qcut(data['distanceToWatercourse'], q=4, labels=["Low", "Mid", "High", "Very High"])

        data["soilType/Elevation"] = (
            data["soilType_encoded"].astype(str) + "/" + bins_elevation.astype(str)
        )
    ...
    def _scale_numerical(
        self, data: pd.DataFrame, scaling_type: str = "standard"
    ) -> pd.DataFrame:
        """
