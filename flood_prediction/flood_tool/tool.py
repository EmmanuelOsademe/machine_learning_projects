...
class Tool(object):
    ...
    def __init__(
        ...
        self._unlabelled_postcodes["postcode"] = self._unlabelled_postcodes[
            "postcode"
        ].apply(self.standardise_UK_postcode)
    ...
    def impute_missing_values(
        ...
        dataframe = data.copy()
        code = dataframe["postcode"]
        dataframe = dataframe.drop(columns=["postcode"])
        if dataframe.isnull().all().any():
            ...
            return pd.concat([code, dataframe], axis=1)

        if method == "mean":
            numeric_col = dataframe.select_dtypes(include=[np.number]).columns
            dataframe[numeric_col] = dataframe[numeric_col].fillna(
                dataframe[numeric_col].mean(numeric_only=True)
            )
        ...
        return pd.concat([code, dataframe_imputed], axis=1)
    ...
    def predict_flood_class_from_postcode(
        ...
        if method == "all_zero_risk":
            ...
            # postcodes_df = pd.DataFrame({"postcode": postcodes})

            # df_postcodes = pd.merge(
            #     postcodes_df,
            #     self._unlabelled_postcodes,
            #     on="postcode",
            #     how="left"
            # )
    ...
    def predict_flood_class_from_OSGB36_location(
        ...
        if method == "all_zero_risk":
            ...
            if len(eastings) != len(northings):
                raise ValueError("eastings and northings must be the same length")

            eastings_northings_df = pd.DataFrame(
                [(e, n) for e, n in zip(eastings, northings)],
                columns=["easting", "northing"],
            )

            df_postcodes = pd.merge(
                eastings_northings_df,
                self._unlabelled_postcodes,
                on=["easting", "northing"],
                how="left",
            )
            ...
            for col in expected_columns:
                if col not in df_postcodes.columns:
                    df_postcodes[col] = np.nan

            df_postcodes = df_postcodes.groupby(
                ["easting", "northing"], as_index=False
            ).first()

            # df_postcodes = df_postcodes.replace({None: np.nan})
    ...
    def predict_flood_class_from_WGS84_locations(
        ...
        if method == "all_zero_risk":
            ...
            eastings = eastings.astype(int)
            northings = northings.astype(int)

            predictions = self.predict_flood_class_from_OSGB36_location(
            ...
            # expected_columns = ['easting','northing','soilType','elevation','nearestWatercourse','distanceToWatercourse','localAuthority']
            # for col in expected_columns:
            #     if col not in df_postcodes.columns:
            #         df_postcodes[col] = np.nan

            # df_postcodes = df_postcodes.groupby(["easting", "northing"], as_index=False).first()
    ...
    def estimate_total_value(self, postal_data: Sequence[str]) -> pd.Series:
        ...
        filtered_data = self._sector_data[
        ...
        return pd.Series(
            data=median_house_prices.values
            * matching_households
            / matching_postcode_units,
    ...
    def estimate_annual_human_flood_risk(
        ...
        return pd.Series(
            data=[
                ...
                for headcount, units, risk_label in zip(
                    matching_headcount, matching_postcode_units, risk_labels
                )
            ],
            index=np.asarray(postcodes),
            name="humanRisk",
